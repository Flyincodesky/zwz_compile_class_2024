...
/*        wl为工作列表,BB为当前传入BB的index*/
int insert_BB_to_WL(WL *wl, int BB){
    if(is_full(wl)) return -1; //判断wl是否已满。     目前算法并不能判断是否已经满了 
...
/*************************Constant_Propagation*****************************/  //284 lines
...
/*在def_vars中找到该sym，返回sym在def_vars中的index(index是def_vars中sym的数量)*/
int find_var(SYM * sym, SYM ** def_vars, int index){
    ...
    printf("can not find a def_var:%s\n",sym->name);
    // int *p;
    // p = 0x0;
    // *p = 1;
    return -1;
...
int CP_for_one_BB(BB **BB_array, int BB, R_node * BB_res, R_node ** res, SYM **def_vars, int def_index){
    ...
    for(cur = BB_array[BB]->in; (cur != NULL) && (cur->prev != BB_array[BB]->out) ; cur = cur->next){
        ...
        if(cur->op == TAC_COPY){
            ...
            else if(cur->b->type = SYM_VAR_INT || cur->b->type == SYM_VAR_CHAR){
        ...
        else if(cur->op >= 1 && cur->op <= 10){ /*运算tac中不包括复制和neg取反*/
            int a_index = find_var(cur->a, def_vars, def_index);
            // a= b op c
            int b_index, c_index;
...
/*数据流分析-常量传播,  输出：res,def_vars,var_index*/
void Constant_Propagation(BB **BB_array, int BB_num, TAC *tac_first, R_node ** res, SYM **def_vars, int *var_index){
    /*BB     **BB_array  : 基本块数组指针
      int      BB_num    : 基本块数量
      TAC     *tac_first : tac链的头指针
      R_node **res       : 返回的数据流分析结果，每个基本块的in out state   ,大小是BB_num
      SYM    **def_vars  : 返回该分析过程中所有的变量列表
      int     *var_index : 返回该分析过程中所有的变量的数量，即 len(def_vars)
    */
    int def_index = 0;
    int flag ;

    //将tac链中所有的var记录在def_vars中。并用def_index记录数量   all the vars
    ...
    *var_index = def_index;
    ...
    //初始化res 为数据流分析做准备   init res data structure. Note that Constant pro need EXTRA data structure!
    for(int i = 0; i < BB_num; i++){
        res[i] = (R_node *)malloc(sizeof(R_node));
        res[i]->n = i;   //给res[i]节点成员变量n赋值基本块id
        // printf("%d,%d\n",BB_array[i]->id,i);
        res[i]->count = def_index;  //给count赋值分析过程变量数量
    ...
    //初始化WL
    WL *wl = init_WL();
    //初始化wl，将程序的基本块index插入wl的data 成员变量 
    for(int i = 0; i < BB_num; i ++){
        if(BB_array[i]->live == 1) insert_BB_to_WL(wl, i);
    }

    while(!is_empty(wl)){
        int tmp_BB = get_BB_from_WL(wl);//出队列一个BB_array index
        // printf("BB- %d\n", tmp_BB);
        // Do RD for this BB
        ...
        if(flag){
            ...
            for(A_node *tmp = BB_array[tmp_BB]->suc; tmp != NULL ; tmp = tmp->next){
                if(tmp->n==BB_array[tmp_BB]->id)continue;
...
void Reaching_definition(BB **BB_array, int BB_num, TAC *tac_first, R_node ** res, TAC ** def_tac, int *def_tac_index, int control_bit){
    ...
    *def_tac_index = def_index;//记录表达式数量

    // TODO: initialize data structure  初始化R_node ** res
    for(int i = 0; i < BB_num; i++){
        res[i] = (R_node *)malloc(sizeof(R_node));
        res[i]->n = i;
...
int LV_for_one_BB(BB **BB_array, int BB, R_node * BB_res, R_node **res, int def_index, SYM **def_vars){
    ...
    for(cur = BB_array[BB]->out; (cur != NULL) && (cur->next != BB_array[BB]->in) ; cur = cur->prev){  
        ...
        else if (cur->op == TAC_ACTUAL || cur->op == TAC_RETURN)//actual a & return a 
        {
            for(int i=0; i<def_index; i++){
                if(cur->a->type==SYM_VAR_INT || cur->a->type==SYM_VAR_CHAR){
                    if(strcmp(def_vars[i]->name,cur->a->name)==0){  
                        tmp_union[i] = 1;
                        break;
                    }
        ...
        else if (cur->op == TAC_IFZ || cur->op == TAC_IF) //ifz b goto a
        {
            if(cur->b->type==SYM_VAR_INT || cur->b->type == SYM_VAR_CHAR){
                for(int i=0; i<def_index; i++){
                    if(strcmp(def_vars[i]->name,cur->b->name)==0){
                        tmp_union[i] = 1;
                        break;
...
/*************************Available_expression*****************************/  //234 lines
/* ldf   判断TAC中的表达式是否与exp_list[1000]中的表达式相同，若相同则返回下标
